# StarUML Controller MCP Server — Flowchart Diagram

An MCP (Model Context Protocol) server specialized for **Flowchart Diagrams**. Enables AI assistants like Claude to programmatically create and manage flowchart diagrams — process nodes (30+ types) and flows in [StarUML](https://staruml.io/).

<p align="center">
  <img src="images/main.jpg" alt="Customer Order Processing — Flowchart diagram created entirely through MCP tools" width="600">
</p>
<p align="center"><em>Example: Customer Order Processing flowchart — created entirely by AI via MCP</em></p>

## Architecture

<p align="center">
  <img src="images/architecture.svg" alt="Architecture: Claude ↔ MCP (stdio) ↔ staruml-controller-flowchart-mcp ↔ HTTP REST API ↔ StarUML" width="800">
</p>
...
```bash
git clone https://github.com/pontasan/staruml-controller-flowchart-mcp.git
cd staruml-controller-flowchart-mcp
npm install && npm run build
```
...
4. The HTTP server starts — all Flowchart Diagram tools become available via MCP
...
```json
{
  "mcpServers": {
    "staruml-flowchart": {
      "command": "node",
      "args": ["/absolute/path/to/staruml-controller-flowchart-mcp/dist/index.js"]
    }
  }
}
```
...
```bash
claude mcp add staruml-flowchart node /absolute/path/to/staruml-controller-flowchart-mcp/dist/index.js
```
...
The core of this server — full CRUD for every Flowchart Diagram resource.

| Resource | Tools |
|---|---|
| **Diagrams** | `flowchart_list_diagrams`, `flowchart_create_diagram`, `flowchart_get_diagram`, `flowchart_update_diagram`, `flowchart_delete_diagram` |
| **Nodes** | `flowchart_list_nodes`, `flowchart_create_node`, `flowchart_get_node`, `flowchart_update_node`, `flowchart_delete_node` |
| **Flows** | `flowchart_list_flows`, `flowchart_create_flow`, `flowchart_get_flow`, `flowchart_update_flow`, `flowchart_delete_flow` |
